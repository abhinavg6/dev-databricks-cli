...
# MAGIC 
# MAGIC Let's now tour the core abstractions in Apache Spark to ensure that you'll be comfortable with all the pieces that you're going to need to understand in order to use Databricks and Spark effectively.
# MAGIC 
...
# MAGIC -   ****The DataFrame****
# MAGIC     -   The DataFrame is a collection of distributed `Row` types. These provide a flexible interface and are similar in concept to the DataFrames you may be familiar with in python (pandas) as well as in the R language.
# MAGIC -   ****The Dataset****
# MAGIC     -   The Dataset is Apache Spark's newest distributed collection and can be considered a combination of DataFrames and RDDs. It provides the typed interface that is available in RDDs while providing a lot of the conveniences of DataFrames. It will be the core abstraction going forward.
# MAGIC 
# MAGIC 
...
# MAGIC 
# MAGIC To read more about some of the other options that are available to users please see the [Databricks Guide on Clusters](https://docs.databricks.com/user-guide/clusters/index.html).
...
# MAGIC 
# MAGIC Transformations are operations that will not be completed at the time you write and execute the code in a cell - they will only get executed once you have called an **action**. An example of a transformation might be to convert an integer into a float or to filter a set of values.
# MAGIC 
# MAGIC ### Actions
# MAGIC 
...
# MAGIC 
# MAGIC The reason for that is because these computations are *lazy* in order to build up the entire flow of data from start to finish required by the user. This is an intelligent optimization for two key reasons. Any calculation can be recomputed from the very source data allowing Apache Spark to handle any failures that occur along the way, and successfully handle stragglers. Secondly, Apache Spark can optimize computation so that data and computation can be `pipelined` as we mentioned above. Therefore, with each transformation Apache Spark creates a plan for how it will perform this work.
# MAGIC 
